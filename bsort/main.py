...
def infer_image(config: Dict[str, Any], image_path: str) -> None:
    ...
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Model file not found at: {model_path}. Please train first or check path.")
...
def main() -> None:
    ...
    parser_train.add_argument('--config', type=str, required=True, help='Path to settings.yaml')

    # --- Subparser for INFER ---
    parser_infer = subparsers.add_parser('infer', help='Run inference on an image')
    parser_infer.add_argument('--config', type=str, required=True, help='Path to settings.yaml')
    parser_infer.add_argument('--image', type=str, required=True, help='Path to image file')
